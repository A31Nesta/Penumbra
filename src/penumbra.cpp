...
namespace pen {
	...
	void init(std::string title, int width, int height, uint32_t penumbra_flags) {
		...
		_window = glfwCreateWindow(1280, 720, "Penumbra", NULL, NULL);
		if (!_window) {
		...
		if (isWayland) {
			...
			init.platformData.nwh = (void*)(struct wl_surface*)glfwGetWaylandWindow(_window);
			init.platformData.type = bgfx::NativeWindowHandleType::Wayland;
		#endif
		} else {
		#ifdef PENUMBRA_X11_COMPAT
			init.platformData.ndt = glfwGetX11Display();
			init.platformData.nwh = (void*)(uintptr_t)glfwGetX11Window(_window);
